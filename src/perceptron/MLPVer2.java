...
public class MLPVer2 {
    ...
    void NegarUltimoDeltaPesoQuick(int tamanoMuestras){
        int cantidadCapas = this.capasNeuronas.size();
        int cantidadNeuronas;
        NeuroninVer2 capaActual[];
        double tamanoDePoblacion = (double)tamanoMuestras;
    ...
    double CalcularErrorPQuick(List<Point2D.Double> puntos_tipo_2, List<Point2D.Double> puntos_tipo_1, List<Point2D.Double> puntos_tipo_0, Point2D.Double Click_acotado) {
        //ImprimirS();
        NeuroninVer2 capaActual[];
        int cantidadCapas = this.capasNeuronas.size();
        double error, deseada[],salida=0;
        deseada = new double[numeroClases];
        //capaSuperior = capaActual;
    ...
    void inicializarQuick(){
        int NumeroCapas = this.capasNeuronas.size();
        int TamanoCapa,CantidadPesos;
        NeuroninVer2 CapaActual[];
        for(int c = 0; c<NumeroCapas;c++){
            CapaActual = this.capasNeuronas.get(c);
            TamanoCapa = CapaActual.length;
            CantidadPesos = CapaActual[0].DeltaWeinght.length;
            for (int n = 0; n<TamanoCapa;n++){
                for (int w = 0; w<CantidadPesos; w++){
                    CapaActual[n].DeltaWeinght[w] = 0.1;
                    CapaActual[n].PrevEp[w] = 0.1;
                }
            }
        }
    }
    ...
    void ActualizarPesosQuick(double learninRate, Point2D.Double Click_acotado) {
        int zeros;
        double crecimiento_maximo = 0.5, Ep=0, sign_e, sign_e_prev;
        
        int CantidadCapas = this.capasNeuronas.size(), CantidadNeuronas, CantidadPesos;
        NeuroninVer2 CapaActual[],capaAnterior[];
        ...
        for (int c = 0; c < CantidadCapas; c++){
            CapaActual = this.capasNeuronas.get(c);
            CantidadNeuronas = CapaActual.length;
            CantidadPesos = CapaActual[0].weights.length;
            
            for(int n = 0; n<CantidadNeuronas; n++){//faltaria un if is first, a ver que hace
                
                //Ep = CapaActual[n].Sencibilidad*capaAnterior[0].fDeNet;
                Ep = CapaActual[n].Sencibilidad*-1;
                sign_e = Math.signum(Ep);
                sign_e_prev = Math.signum(CapaActual[n].PrevEp[0]);
                
                if(sign_e == sign_e_prev || Ep >= CapaActual[n].PrevEp[0]){//para evitar div 0
                    CapaActual[n].DeltaWeinght[0] = CapaActual[n].DeltaWeinght[0]*crecimiento_maximo;
                   //CapaActual[n].DeltaWeinght[0] = CapaActual[n].DeltaWeinght[0]*.01;
                   
                }else{
                    CapaActual[n].DeltaWeinght[0] = learninRate*(-Ep*CapaActual[n].DeltaWeinght[0])/(CapaActual[n].PrevEp[0]-Ep);
                    if (sign_e == sign_e_prev){
                        CapaActual[n].DeltaWeinght[0] += -Ep;
                    }
                }
                CapaActual[n].PrevEp[0] = Ep;
                
                //CapaActual[n].DeltaWeinght[0] = 
                
                for (int w = 1; w < CantidadPesos;w++){
                    Ep = CapaActual[n].Sencibilidad*capaAnterior[w-1].fDeNet;//en la capa anterior no hay neurona para cada peso, la 0 va al peso 1
                    sign_e = Math.signum(Ep);
                    sign_e_prev = Math.signum(CapaActual[n].PrevEp[w]);
                
                    if(sign_e == sign_e_prev && Ep >= CapaActual[n].PrevEp[w]){//para evitar div 0
                    CapaActual[n].DeltaWeinght[w] = CapaActual[n].DeltaWeinght[w]*crecimiento_maximo;
                    //CapaActual[n].DeltaWeinght[w] = CapaActual[n].DeltaWeinght[w]*.001;
                    }else{
                    CapaActual[n].DeltaWeinght[w] = learninRate*(-Ep*CapaActual[n].DeltaWeinght[w])/(CapaActual[n].PrevEp[w]-Ep);
                    if (sign_e == sign_e_prev){
                        CapaActual[n].DeltaWeinght[w] += -Ep;
                    }
                }
                CapaActual[n].PrevEp[w] = Ep;
            ...
            for (int n = 0; n< CantidadNeuronas; n++){
                for (int w = 0; w < CantidadPesos; w++){
                    if(CapaActual[n].DeltaWeinght[w]>crecimiento_maximo){
                        CapaActual[n].weights[w] += crecimiento_maximo;
                    }else
                        if(CapaActual[n].DeltaWeinght[w]<-crecimiento_maximo){
                          CapaActual[n].weights[w] += -crecimiento_maximo;  
                        }else{
                    CapaActual[n].weights[w] += (CapaActual[n].DeltaWeinght[w]);
                        }
                }
            }
    ...
    void EntrenarRedQuick(List<Point2D.Double> puntos_tipo_2, List<Point2D.Double> puntos_tipo_1, List<Point2D.Double> puntos_tipo_0, List<Point2D.Double> Click_acotados, int maximasGeneraciones, double error2MaximoAcumulado, double learningRate) {
        ...
        inicializarQuick();//1111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111
        ...
        while (generacionActual < maximasGeneraciones && this.error2AcumuladoDeUltimaGen > error2MaximoAcumulado) {
            ...
            for (int numeroClick = 0; numeroClick < Click_acotados.size(); numeroClick++) {
                puntoActual = Click_acotados.get(numeroClick);
                this.Forward(puntoActual);//2222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222
                this.Backward(puntos_tipo_2, puntos_tipo_1, puntos_tipo_0, puntoActual);//aqui ya se actualizo el error2acumulado a el correspondiente a esta generacion222
                
                this.ActualizarPesosQuick(learningRate, puntoActual);//3333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333
            ...
            System.out.println("\nFin de la generacion: " + generacionActual + " | Con un error2 de: " + this.error2AcumuladoDeUltimaGen);
            //DIBUJAR GRAFICA AL FINAL
            errorTotaldeGeneraciones[generacionActual] = error2AcumuladoDeUltimaGen/Click_acotados.size();
            //errorGraficaArray(generacionActual, error2AcumuladoDeUltimaGen);
